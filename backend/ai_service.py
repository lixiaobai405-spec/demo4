...
def chat(messages: list[dict], context: str = "", force_summary: bool = False) -> str:
    ...
    user_prompt = f"""正在进行领导力建模信息采集。需收集7项：行业/业务、规模/阶段、战略重点、管理痛点、建模层级、优秀管理者画像、标准库参照。
...
def build_behavior_contrast(dim_name: str) -> tuple[list[str], list[str]]:
    """生成独立于 BARS 档位的正负向行为清单。

    BARS 是评分锚点；正负向行为是 360 反馈/访谈校准用的观察清单，不能直接拿 5/3/2/1 档文本拼凑。
    """
    name = dim_name or "该维度"
    positives = [
        f"围绕{name}提前说明目标、责任人和交付标准，让相关成员知道要完成什么、何时完成、如何判断完成。",
        f"在{name}相关任务推进中主动同步风险、依赖和进展变化，必要时组织关键人员及时对齐。",
        f"任务结束后基于事实复盘{name}相关行为效果，明确保留做法、修正动作和下一次跟进节点。",
    ]
    negatives = [
        f"只在{name}相关问题暴露后临时催办，没有提前澄清目标、边界、责任和风险。",
        f"遇到{name}相关分歧时只传递信息或转发要求，没有推动关键人员形成共同判断。",
        f"事后缺少基于事实的复盘和跟进，使{name}相关问题在相似场景中反复出现。",
    ]
...
- BARS 和正负向行为是两种不同产物：BARS 用于评分分级，正负向行为用于360反馈和访谈校准
- 禁止把 BARS 的5/4/3分直接改写成正向行为，也禁止把2/1分直接改写成负向行为
- 正负向行为对照必须直接服务于360度评估反馈，不是泛泛评价；每条应是可观察的行为清单，而不是评分档位句
...
def build_rule_based_anchors(dimensions: list, critical_incidents: str = "") -> list[dict]:
    ...
    for idx, dim in enumerate(dimensions or [], 1):
        ...
        positives, negatives = build_behavior_contrast(dim_name)
        result.append({
            "dimension_id": dim_id,
...
def normalize_anchor_item(item: dict) -> dict:
    ...
    fallback_positives, fallback_negatives = build_behavior_contrast(dim_name)
    if not positives:
        positives = fallback_positives
    if not negatives:
